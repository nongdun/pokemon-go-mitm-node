...
class PokemonGoMITM
  ...
  handleProxyRequest: (ctx, callback) =>
    console.log "[+] Request to " + ctx.clientToProxyRequest.headers.host
    if ctx.clientToProxyRequest.headers.host is @endpoint
      requested = []
      requestChunks = []
      responseChunks = []
      ...
      ctx.onRequestEnd (ctx, callback) =>
        ...
        wstream = fs.createWriteStream('dumps/' + Date.now() + '.request');
        ...
        console.log "[+] RequestEnvelope found: ", data
        ...
        ctx.proxyToServerRequest.write buffer2
      ...
      ctx.onResponseEnd (ctx, callback) =>
        ...
        wstream = fs.createWriteStream('dumps/' + Date.now() + '.response');
        wstream.write(buffer)
        wstream.end()
        try
          data = POGOProtos.parseWithUnknown buffer, @responseEnvelope
        catch e
          @log "[-] Parsing protobuf of ResponseEnvelope failed.."
          ctx.proxyToClientResponse.end buffer
          return callback()

        console.log "[+] ResponseEnvelope found: ", data
        ...
        for id,response of data.returns
          ...
          if proto in POGOProtos.info()
            ...
            console.log "[+] Response #{protoId} found: ", decoded
        ctx.proxyToClientResponse.end buffer
        callback()
    ...
    return rp(
      ...
      headers:
        ...
        try
          ...
          decoded = POGOProtos.parseWithUnknown buffer, "POGOProtos.Networking.Envelopes.ResponseEnvelope"
